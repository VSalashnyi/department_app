let state, current_department;


//import { state, current_department } from './variables';
import { getDepartments } from "./getDepartments";


//---------------------------------------------------------controller---------

class DepartmentAppController {
  constructor(departmentAppView){
   ...
   async init(){
    ...
    this.departmentAppView.renderNavigation();
    this.departmentAppView.render(window.location.pathname);
  }
}

//-----------------------------------------------------------view--------------

class DepartmentAppView{

  renderNavigation() {
    $('nav').empty();

    state.forEach(dep => {
      this.renderButton(dep.department_title, 'btn btn-secondary m-3', 'nav', () => this.navigate(dep.id))
    });
  ...
  render(route){
    const Route = {
      '/' : () => this.renderPage('Choose department you are interesting in, or add your own.'),
      '/no-dep' : () => this.renderPage('No department left. Add new department.'),
    ...
    if(Route[route]){
      ...
      current_department = state.find(department => department.id === +route.substring(1));
      this.renderContent();
    }
  }
